...
public static class CharacterFeatures
{
	...
	{
		BonkMenu.Core.Log.Info($"[UnlockCharacter] Unlocking: {characterName} (ID: {characterId})");
		try
		{
			...
			BonkMenu.Core.Log.Info($"[UnlockCharacter] Attempted: {characterName} (ID: {characterId})");
		}
		catch (Exception ex)
		{
